...
// artifactMeta travels beside the body. It is what list shows and what a reader is
...
// storeClient talks to the artifact worker. Its zero value is not usable; build
// one with newClient.
...
func newClient(baseURL string, keys *keySource, ttl time.Duration, maxBody int64) *storeClient {
	return &storeClient{
...
func (c *storeClient) Link(id string) (string, error) {
	key, err := c.keys.get()
	if err != nil {
		return "", err
	}
...
func (c *storeClient) IndexLink() (string, error) {
	key, err := c.keys.get()
	if err != nil {
		return "", err
	}
...
func (c *storeClient) Publish(ctx context.Context, id string, body []byte, meta artifactMeta) (string, error) {
...
func (c *storeClient) Fetch(ctx context.Context, target string) ([]byte, artifactMeta, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, artifactMeta{}, statusError(resp)
	}
	...
	if len(body) > readLimit {
		return nil, artifactMeta{}, fmt.Errorf("artifact is larger than %d bytes, too large to read into this conversation; open it at %s", readLimit, address)
	}
	meta := artifactMeta{ContentType: resp.Header.Get("Content-Type"), Bytes: int64(len(body))}
...
func (c *storeClient) List(ctx context.Context, limit int, by string) ([]artifactMeta, error) {
	key, err := c.keys.get()
	if err != nil {
		return nil, err
	}
...
func (c *storeClient) address(target string) (string, error) {
...
func (c *storeClient) do(req *http.Request, out any) error {
	resp, err := c.http.Do(req)
	if err != nil {
		return err
	}
...
func decodeMeta(header string) (artifactMeta, error) {
