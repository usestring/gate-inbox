...
// Board.Transcript answers with one, and so does a ToolDriver's
// MigrateTranscript for a CLI the core has no code for.
type Transcript struct {
	// Kind is the layout: "claude" and "codex" are files of one JSON
	// record per line, "opencode" is a JSON document Command prints. It is
	// empty for a CLI a tool driver teaches the board, whose Format says
	// the layout instead.
	Kind string
	...
	Command string
	// Format tells the agent taking over how the transcript is laid out,
	// in prose. A tool driver sets it; the board's own CLIs leave it empty.
	Format string
}
