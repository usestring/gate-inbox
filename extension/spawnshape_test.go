...
func TestSessionHooksJoinEveryShapersPrefixInOrder(t *testing.T) {
	rules := &shaper{id: "rules", shape: extension.SpawnShape{PromptPrefix: "  Follow the house rules.\n", PromptSuffix: "Report when done.\n"}}
	...
	hooks := sessionHooks(t, rules, blank, &stub{id: "tools-only"}, keep)
	...
	for _, s := range []*shaper{rules, blank, keep} {
		if len(s.asked) != 1 || !reflect.DeepEqual(s.asked[0], launch) {
...
func TestSessionHooksTreatAFailingShaperAsARefusal(t *testing.T) {
	...
	hooks := sessionHooks(t, &shaper{id: "budget", refuse: errors.New("no rules to add")}, later)
	_, err := hooks.ShapeSpawn(context.Background(), extension.Launch{})
	if err == nil || !strings.Contains(err.Error(), `extension "budget" refused the launch: no rules to add`) {
