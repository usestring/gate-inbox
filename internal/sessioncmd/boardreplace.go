...
func (s *Sessions) prepareReplace(runtime *runtime, targetID, rolePrefix string, opts BoardLaunchOptions, rehearse bool) (store.Session, boardLaunch, error) {
	...
	if old.Archived {
		return store.Session{}, boardLaunch{}, fmt.Errorf("session %s is archived; unarchive it before replacing it", old.ID)
	}
	if old.Role != "" && !strings.HasPrefix(old.Role, rolePrefix) {
		return store.Session{}, boardLaunch{}, fmt.Errorf("session %s plays %s, another extension's role; only that extension may replace it", old.ID, old.Role)
	}
