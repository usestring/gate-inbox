...
// roleSends is every operator send roleBoard heard, reset by useRoles.
var (
	roleSendsMu sync.Mutex
	roleSends   []extension.OperatorSend
)

func (roleBoard) OperatorSent(_ context.Context, send extension.OperatorSend) (string, error) {
	roleSendsMu.Lock()
	defer roleSendsMu.Unlock()
	roleSends = append(roleSends, send)
	return "", nil
}
...
// A relayed line and the operator's own shell send are both the operator's
// words, delivered unfenced, but only the shell send is queued as one the
// board reports to extensions when it is typed in: the relaying extension
// recorded the relayed line already.
func TestARelayQueuesAsRelayedAndAShellSendAsTheOperator(t *testing.T) {
	h := newSessionHarness(t)
	useRoles(t)
	reviewer := helperOf(t, h, h.caller.ID, "board/reviewer")
	if _, err := h.sessions.Send(reviewer.ID, h.caller.ID, "use the second draft", "", false); err != nil {
		t.Fatal(err)
	}
	if _, err := h.sessions.SendAsHuman("", h.caller.ID, "yes, ship it", "", false); err != nil {
		t.Fatal(err)
	}
	queued, err := h.store.Inbox(h.caller.ID, store.InboxFilter{Limit: 10})
	if err != nil {
		t.Fatal(err)
	}
	senders := map[string]string{}
	for _, msg := range queued {
		senders[msg.Body] = msg.SenderID
	}
	if got := senders["use the second draft"]; got != store.RelayedHumanSenderID {
		t.Fatalf("the relay queued as %q, want %q (senders %v)", got, store.RelayedHumanSenderID, senders)
	}
	if got := senders["yes, ship it"]; got != store.HumanSenderID {
		t.Fatalf("the shell send queued as %q, want %q (senders %v)", got, store.HumanSenderID, senders)
	}
	for body, sender := range senders {
		if !store.SpeaksAsOperator(sender) {
			t.Fatalf("%q queued as %q, which is not delivered as the operator's words", body, sender)
		}
	}
	// Only the shell send reaches the operator-send handlers: the relayed
	// line was the relaying extension's to record.
	roleSendsMu.Lock()
	heard := slices.Clone(roleSends)
	roleSendsMu.Unlock()
	if len(heard) != 1 || heard[0].Text != "yes, ship it" {
		t.Fatalf("the send handlers heard %+v, want only the shell send", heard)
	}
}
