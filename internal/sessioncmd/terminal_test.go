...
`
	...
	h.terminals = newTerminals(configDir, MCPVocabulary(), func(string) (*tmux.Driver, error) { return driver, nil })
	t.Cleanup(func() {
...
func TestTerminalCommandsRejectUnsafeTargetsAndInvalidInput(t *testing.T) {
	...
	if _, err := h.terminals.List(""); err == nil || !strings.Contains(err.Error(), "GATE_INBOX_SESSION_ID") {
