...
// inBox reports whether up (dir -1) or down (dir 1) moves the cursor within
// a multi-line box rather than off it: true unless the cursor is already on
// the box's first row going up or its last row going down, counting the rows
// a long line wraps onto. Every other field hands up and down to the form.
func (f *viewField) inBox(dir int) bool {
	if !f.typed() || !f.spec.Multiline {
		return false
	}
	info := f.box.LineInfo()
	if dir < 0 {
		return f.box.Line() > 0 || info.RowOffset > 0
	}
	return f.box.Line() < f.box.LineCount()-1 || info.RowOffset < info.Height-1
}
...
func (m *Model) handleViewFieldKey(msg tea.KeyPressMsg, action keymap.Action, bound bool) (bool, tea.Cmd) {
	...
	key := msg.String()
	switch {
	case key == "tab", key == "down" && !field.inBox(1):
		return true, fields.move(1)
	case key == "shift+tab", key == "up" && !field.inBox(-1):
		return true, fields.move(-1)
...
func (m *Model) fieldLines(inner int) (lines []string, starts []int) {
	...
	for _, id := range fields.order {
		if f := fields.byID[id]; !f.spec.Multiline {
			labelWidth = max(labelWidth, textfmt.Width(cleanText(f.spec.Label)))
		}
	}
	...
	for i, id := range fields.order {
		...
		label := textfmt.TruncateWidth(cleanText(f.spec.Label), fieldLabelMax, "…")
		switch {
		case f.spec.Multiline:
	...
	for i, line := range lines {
		lines[i] = textfmt.TruncateWidth(line, inner, "…")
	}
	return lines, starts
}
...
func (m *Model) fieldHint() [][2]string {
	...
	return [][2]string{{"tab/↑↓", "move"}, {submit, "submit"}}
}
