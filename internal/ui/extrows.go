...
type ExtensionFilter struct {
	...
	// Aliases are action names the key file may still carry for this
	// filter's key.
	Aliases []string
	// CarriedFrom are settings the filter's state used to be kept under,
	// read in order while the filter has no state of its own.
	CarriedFrom []string
}
...
func (m *Model) newListFilter(owner string, filter ExtensionFilter) *listFilter {
	...
	if m.store != nil {
		key := filterSetting(listed)
		value, err := m.store.Setting(key)
		if err == nil && value == "" {
			value = m.carryFilterSetting(listed, key)
		}
		listed.on = value == "on"
	}
	return listed
...
// carryFilterSetting is a filter's state taken from the first setting it
// used to be kept under that holds one, written under the filter's own key
// so the carry happens once: from then on the filter's own key answers, and
// a later change to the old setting is not read again. The old setting is
// left as it is. It belongs to whatever stored it, and a build that still
// reads it, such as one without this extension, keeps working.
func (m *Model) carryFilterSetting(f *listFilter, key string) string {
	for _, old := range f.CarriedFrom {
		if old == "" || old == key {
			continue
		}
		stored, err := m.store.Setting(old)
		if err != nil || stored == "" {
			continue
		}
		value := "off"
		...
		if err := m.store.SetSetting(key, value); err != nil {
			logging.Warn("could not carry an extension filter's setting over",
				"extension", f.owner, "action", f.Action, "from", old, logging.Err(err))
		}
		return value
	}
	return ""
...
func (m *Model) extensionFilterBadges(badge func(label, key, action string)) {
	for _, f := range m.extFilters {
		...
		badge(strings.ToUpper(textfmt.TruncateWidth(label, 16, "…")), m.tightCap(keymap.ContextList, keymap.Action(f.Action)), "show all")
	}
}
