...
// opened is how many views have been asked for through Open, counted as
// each call is made rather than when its message reaches the board.
func (b *ExtensionBridge) opened() int {
	b.mu.Lock()
	defer b.mu.Unlock()
	return b.views
}
...
func (m *Model) viewExtension() (frame string) {
	...
	for _, row := range rows[:min(len(rows), bodyRoom)] {
		...
		lines = append(lines, textfmt.TruncateWidth(b.String(), inner, "…"))
	}
	if len(fieldBlock) > 0 {
