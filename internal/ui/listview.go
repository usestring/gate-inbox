...
func (m *Model) renderSessionEntry(entry treeRow, selected bool, width int, pad, guides, trail, bg string) string {
	...
	// Where an extension's badge that asks to follow the name goes in.
	nameEnd := len(head)
	if queued := m.queuedMessages[sess.ID]; queued > 0 {
	...
	// about the row itself, and take only the room that is left. One that
	// asks to follow the name is drawn there instead when that room allows,
	// so it never cuts the name or pushes out the board's own badges.
	if m.stackedRows() {
		after, slot := m.extensionBadges(sess.ID, width-textfmt.Width(head), width-textfmt.Width(indent)-textfmt.Width(meta), false)
		head = head[:nameEnd] + after + head[nameEnd:]
		meta += slot
	} else {
		room := width - railGutter - textfmt.Width(head) - 2 - textfmt.Width(meta)
		after, slot := m.extensionBadges(sess.ID, room, room, true)
